...
Order->Dessert = PIE;
  } else if((VCAST_STATEMENT_INSTRUMENTATION_POINT_REALTIME(vcast_unit_stmt_bytes_9,9,2),Order->Entree == LOBSTER &&
            Order->Salad == GREEN &&
            Order->Beverage == WINE)) {
...
/*vcast_internal_end*/
/*vcast_internal_start*/{/*vcast_internal_end*/
  int i = (VCAST_STATEMENT_INSTRUMENTATION_POINT_REALTIME(vcast_unit_stmt_bytes_9,9,36),0);
  if((VCAST_STATEMENT_INSTRUMENTATION_POINT_REALTIME(vcast_unit_stmt_bytes_9,9,37),WaitingListSize > 9))
    {
